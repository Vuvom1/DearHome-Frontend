...
const EditGoodReceivedNote = () => {
    ...
    const fetchGRNData = async () => {
        try {
            ...
            const response = await goodReceivedNoteApiRequest.getById(id);
            ...
            // Set form values
            form.setFieldsValue({
                date: moment(data.receivedDate),
                shippingCost: data.shippingCost || 0,
                note: data.note || '',
            });
            
            // Set items
            if (data.goodReceivedItems && data.goodReceivedItems.$values) {
                const formattedItems = data.goodReceivedItems.$values.map(item => {
                    // Check if item has $values structure or direct properties
                    const itemData = item.$values || item;
                    return {
                        key: itemData.id,
                        id: itemData.id,
                        ...
                        variantInfo: itemData.variant ? itemData.variant : null
                    };
                });
            ...
            message.error('Failed to fetch GRN data');
        } finally {
    ...
    const handleSubmit = async (values) => {
        try {
            ...
            // Make the API call
            const response = await goodReceivedNoteApiRequest.updateGoodReceivedNote(payload);
            
            message.success('Goods Received Note updated successfully');
            
            // Navigate back to inventory page
            navigate(URLS.ADMIN.INVENTORY);
        } catch (error) {
            console.error('Error updating GRN:', error);
            message.error('Failed to update Goods Received Note');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div>
            <Flex justify="space-between" align="center" style={{ marginBottom: 16 }}>
                <Typography.Title level={2}>Edit Goods Received Note</Typography.Title>
                <Flex gap={16}>
                    <Button
                        variant="outlined"
...
export default EditGoodReceivedNote;
